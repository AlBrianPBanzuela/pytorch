...
def build_torchcomms(
    ...
    """Build torchcomms extensions via CMake.

    Args:
        ext: The CMake extension being built.
        build_ext: The setuptools build_ext command instance.
        cmake_cache_vars: Parsed CMake cache variables.
        cmake_command: Path to the cmake binary.
        report: Callable for logging build messages.
        is_windows: Whether building on Windows.
    """
    ...
    if build_ext.inplace or getattr(build_ext, "editable_mode", False):  # type: ignore[attr-defined]
        ...
        for artifact_name in (
            ...
            if built_artifact.exists():
                build_ext.copy_file(built_artifact, TORCHCOMMS_SOURCE_DIR / artifact_name)  # type: ignore[attr-defined]
